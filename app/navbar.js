  ...
  return (
    <main className={styles.main}>
      ...
      <ul class="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-transparent dark:bg-transparent md:dark:bg-transparent dark:border-gray-700">
        <li>
          <NavLink  to={"/"} className={`${MontTitle.className} [&.active]:text-white block py-2 pl-3 pr-4 text-gray-700 bg-white rounded md:bg-transparent md:text-white md:p-0 dark:text-white`} aria-current="page">Inicio</NavLink>
        </li>
        <li>
          <NavLink  to={"/"} className={`${MontTitle.className} [&.active]:text-white-500 block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent`}>Serviços</NavLink>
        </li>
        <li>
          <NavLink  to={"/"} className={`${MontTitle.className}  [&.active]:text-white-500 block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent`}>Projetos</NavLink>
        </li>
        <li>
          <NavLink  to={"/"} className={`${MontTitle.className}  [&.active]:text-white-500 block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent`}>Sobre</NavLink>
        </li>
      </ul>
    </div>
          </div>
